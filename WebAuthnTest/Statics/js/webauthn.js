...
const register = () => {
    ...
    navigator.credentials.create({ publicKey })
        .then(function (newCredentialInfo) {
            ...
            document.getElementById('register_pubKey').value = JSON.stringify(attestation.authData.credentialPublicKey, null, 2);
        }).catch(function (err) {
            console.log(err);
        });
};
...
const authn = () => {
    ...
    navigator.credentials.get({ publicKey })
        .then(function (assertion) {
            ...
            document.getElementById('authn_pubKey').value = JSON.stringify(authenticator.credentialPublicKey, null, 2);
        }).catch(function (err) {
            console.log(err);
        });
};
